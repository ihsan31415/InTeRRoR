...
L298N:
  ...
  - ENA: 5
  - ENB: 6
  - Library: do not use library for this components!
...
def compile_code(kode):
    try:
        ...
        # Compile/verify kode menggunakan arduino-cli
        result = subprocess.run(
            ["arduino-cli", "compile", "--fqbn", "arduino:avr:uno", "--verify", "ardu.ino"],
            capture_output=True,
            text=True
        )
        # Return hasil kompilasi
...
def upload_to_arduino(kode):
    ...
    try:
        ...
        upload_result = subprocess.run(
            ["arduino-cli", "upload", "--fqbn", "arduino:avr:uno", "--port", port, "ardu.ino"],
